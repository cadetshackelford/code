...
post '/numplayers' do
 ...
 if playerchoice == 'Single'
    ...
    player1 = params[:player1]
    player2 = params[:player2]
    erb :guessing, :layout => false, locals: {p1_name: player1, p2_name: player2, blank: session[:game].correct_blank, array: session[:game].guessed,message: "pick a letter"}
end
    post '/guess' do
    ...
    if session[:game].already_guessed(choice) == true
        ...
        erb :guessing,:layout => false, locals: {p1_name: player1, p2_name: player2, blank: session[:game].correct_blank, array: session[:game].guessed,message: "thats already been picked"}
    end
end
 get "/lose" do
     player1 = params[:player1]
    player2 = params[:player2]
     erb :lose,:layout => false, locals: {p1_name: player1, p2_name: player2, word: session[:game].name,counter:session[:game].counter}

 end
 get "/winner" do
     player1 = params[:player1]
    player2 = params[:player2]
     erb :winner,:layout => false, locals: {p1_name: player1, p2_name: player2, word: session[:game].name,counter:session[:game].counter}
 end
